...
class ACAccessory {
  constructor(platform, accessory, client, mac) {
    ...
    // NOTE: the Home app orders an accessory's tiles by its own rules (it clusters the same-type mode
    // switches), so building the Fan here does NOT control where its tile lands — tile order isn't ours to set.
