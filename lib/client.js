...
class SmartHQClient extends EventEmitter {
  constructor(log, username, password) {
    ...
    this.queues = new Map(); // mac -> { writes: Map(erdNorm -> {value, waiters[]}), running }
  ...
  // Public write: enqueue into the per-appliance serialized + coalesced queue. A HomeKit scene fires
  // many writes at once; queuing them (deduped by ERD, sent one at a time, power-first) avoids the
  // races and "superseded" rejections that left scenes only half-applied.
  setErd(mac, erd, value) {
    const e = C.normErd(erd);
    let q = this.queues.get(mac);
    if (!q) { q = { writes: new Map(), running: false }; this.queues.set(mac, q); }
    return new Promise((resolve, reject) => {
      const cur = q.writes.get(e);
      if (cur) {
        // Coalesce same-register writes within a burst. For POWER, ON beats OFF so a scene turning
        // the unit on can't be flipped off by a deselected mode switch's (stale) power-off write.
        if (e === C.normErd(C.ERD.POWER)) cur.value = (cur.value === C.POWER.ON || value === C.POWER.ON) ? C.POWER.ON : value;
        else cur.value = value;
        cur.waiters.push({ resolve, reject });
      } else {
        q.writes.set(e, { value, waiters: [{ resolve, reject }] });
      }
      if (!q.running) { q.running = true; setTimeout(() => this._drain(mac), C.WRITE_COALESCE_MS); }
    });
  }

  // Next ERD to send: POWER first (so the unit is on before mode/temp/fan), then OPERATION_MODE, then the rest.
  _nextErd(writes) {
    const pwr = C.normErd(C.ERD.POWER), mode = C.normErd(C.ERD.OPERATION_MODE);
    if (writes.has(pwr)) return pwr;
    if (writes.has(mode)) return mode;
    return writes.keys().next().value;
  }

  async _drain(mac) {
    const q = this.queues.get(mac);
    if (!q) return;
    while (q.writes.size) {
      const e = this._nextErd(q.writes);
      const item = q.writes.get(e);
      q.writes.delete(e);
      try { await this._sendErd(mac, e, item.value); item.waiters.forEach((w) => w.resolve()); }
      catch (err) { item.waiters.forEach((w) => w.reject(err)); }
      if (q.writes.size) await this._delay(e === C.normErd(C.ERD.POWER) ? C.POWER_SETTLE_MS : C.WRITE_GAP_MS);
    }
    q.running = false;
  }
  ...
  // Send one setErd over the websocket; resolves on backend ack. Optimistic local apply for snappy UI.
  _sendErd(mac, e, value) {
